"""
The 'CryptoWeeklyPopupWindow' class represents a Tkinter popup window for displaying
 weekly cryptocurrency data and charts.

Class Methods:
- '__init__(self, master, title)': Initializes a new 'CryptoWeeklyPopupWindow' instance.
...
- 'display_chart(self, df, currency)': Displays a candlestick chart based on the provided DataFrame and currency.
    - 'df': The DataFrame containing weekly cryptocurrency data.
    - 'currency': The currency abbreviation for the chart.
- 'refresh_results(self)': Refreshes the displayed cryptocurrency data and chart based on user inputs
 and performs input validation.

Note:
- This class represents a Tkinter popup window for interacting with weekly cryptocurrency data.
...
class CryptoWeeklyPopupWindow(tk.Toplevel):
    ...
    def refresh_results(self):
        try:
            ...
            if is_valid_date is None and is_valid_currency is None:
                df = self.method.weekly_crypto_report(crypto=crypto)
                self.tree_view(df)
                self.display_chart(df, currency=currency)
            elif is_valid_currency is None and is_valid_date is True:
                df = self.method.weekly_crypto_report(crypto=crypto, date=date)
                self.tree_view(df)
                self.display_chart(df, currency.upper())
            elif is_valid_currency is True and is_valid_date is None:
                df = self.method.weekly_crypto_report(crypto=crypto, currency=currency)
                self.tree_view(df)
                self.display_chart(df, currency.upper())
            elif is_valid_currency is True and is_valid_date is True:
                df = self.method.weekly_crypto_report(crypto=crypto, currency=currency, date=date)
                self.tree_view(df)
                self.display_chart(df, currency.upper())
